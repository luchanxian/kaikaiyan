...
- (void)_loadVideoData{
    ...
    NSData *data = [ud objectForKey:@"dailyArray"];
    if (data) {
        self.videoTable.dailyListModelArray = [NSKeyedUnarchiver unarchiveObjectWithData:data];
    ...
    [DataService requestAFUrl:@"http://baobab.wandoujia.com/api/v1/feed?num=10&date=20190929&vc=125&u=213367ae25cb8116060ddbd038303c56853d00ea&v=1.8.1&f=iphone" httpMethod:@"GET" params:nil data:nil block:^(id result) {
        ...
        NSData *data = [NSKeyedArchiver archivedDataWithRootObject:_dailyArray];
        NSUserDefaults *ud = [NSUserDefaults standardUserDefaults];
        [ud setObject:data forKey:@"dailyArray"];
        [ud synchronize];
